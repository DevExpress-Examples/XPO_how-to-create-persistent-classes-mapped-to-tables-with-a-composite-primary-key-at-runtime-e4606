...
Namespace XpoConsoleApplication
    ...
    Friend Class Program
        ...
        Shared Sub Main(ByVal args As String())
            ...
            Dim miAssignmentKey As XPComplexCustomMemberInfo = New XPComplexCustomMemberInfo(ciAssignment, "Key", GetType(Object), New KeyAttribute())
            ...
            Dim miTaskKey As XPComplexCustomMemberInfo = New XPComplexCustomMemberInfo(ciTask, "Key", GetType(Object), New KeyAttribute())
            ...
            XpoDefault.DataLayer = New SimpleDataLayer(dict, New DB.InMemoryDataStore())
            'Create DB and some sample data
            Using uow As UnitOfWork = New UnitOfWork()
                uow.UpdateSchema()
                Dim engineers As IList(Of Object) = CreateObjects(uow, ciEngineer, New String() {"UserID", "PublicName"}, New Object()() {New Object() {"U1", "John"}, New Object() {"PAUL", "Paul"}, New Object() {"XFR", "Fred"}})
                Dim projects As IList(Of Object) = CreateObjects(uow, ciProject, New String() {"ProjectID", "Title", "Description"}, New Object()() {New Object() {"P1", "DemoApp", "bla-bla-bla"}, New Object() {"P2", "MegaApp", "duh!"}, New Object() {"PNEW", "KillerApp", "wow!"}})
                Dim assignments As IList(Of Object) = CreateObjects(uow, ciAssignment, New String() {"Key.Engineer", "Key.Project", "Role", "Rank"}, New Object()() {New Object() {engineers(1), projects(0), "Developer", 6}, New Object() {engineers(2), projects(0), "Developer", 8}, New Object() {engineers(0), projects(1), "Leader", 10}, New Object() {engineers(2), projects(1), "Developer", 8}, New Object() {engineers(1), projects(1), "Consultant", 3}, New Object() {engineers(1), projects(2), "Developer", 8}, New Object() {engineers(2), projects(2), "Developer", 7}, New Object() {engineers(0), projects(2), "Tester", 5}})
                Dim tasks As IList(Of Object) = CreateObjects(uow, ciTask, New String() {"Key.Project", "Key.TaskNo", "Description"}, New Object()() {New Object() {projects(0), 1, "Start"}, New Object() {projects(0), 2, "Finish"}, New Object() {projects(1), 0, "Framework"}, New Object() {projects(1), 1, "Feature1"}, New Object() {projects(1), 2, "Feature2"}, New Object() {projects(1), 3, "Feature3"}, New Object() {projects(1), 4, "Feature4"}, New Object() {projects(1), 5, "Feature5"}, New Object() {projects(1), 6, "Polish"}, New Object() {projects(2), 1, "ready"}, New Object() {projects(2), 2, "steady"}, New Object() {projects(2), 3, "go"}})
                uow.CommitChanges()
            End Using
            ...
            Using uow As UnitOfWork = New UnitOfWork()
                '
                Dim xpv As XPView = New XPView(uow, ciTask)
                xpv.Sorting.Add(New SortProperty("Project", DB.SortingDirection.Ascending))
                xpv.Sorting.Add(New SortProperty("No", DB.SortingDirection.Ascending))
            ...
            Using uow As UnitOfWork = New UnitOfWork()
                '
                Dim xpc As XPCollection = New XPCollection(uow, ciProject)
            ...
            Using uow As UnitOfWork = New UnitOfWork()
                ...
                Dim taskKey As DevExpress.Xpo.Helpers.IdList = New DevExpress.Xpo.Helpers.IdList()
                taskKey.Add(project)
                taskKey.Add(2)
        ...
        Private Shared Function DumpValues(ByVal ci As XPClassInfo, ByVal o As Object, ByVal props As String()) As String
            ...
            For Each p As String In props
                If sb.Length > 0 Then sb.Append(", ")
                Dim v As Object = New DevExpress.Data.Filtering.Helpers.ExpressionEvaluator(ci.GetEvaluatorContextDescriptor(), New OperandProperty(p)).Evaluate(o)
                sb.Append(v)
            Next
